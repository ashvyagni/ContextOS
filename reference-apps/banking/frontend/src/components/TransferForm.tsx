...
export function TransferForm() {
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`Transfer successful: ${tx.amount}`);
      setAmount('');
      setDescription('');
    } catch (e: any) {
      setError(e.message);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#7b654b]">Internal move</p>
          <h2 className="mt-2 text-3xl text-[#171410]" style={{ fontFamily: 'Fraunces, Georgia, serif' }}>
            Transfer funds
          </h2>
        </div>
        <div className="rounded-full border border-[#c9d7db] bg-[#e9eff2] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-[#3b5362]">
          Secure transfer
        </div>
      </div>

      <form onSubmit={handleTransfer} className="space-y-5 rounded-[28px] border border-[#d9c9ae] bg-[#f9f5ee] p-5 shadow-[0_14px_28px_rgba(17,16,14,0.04)] sm:p-6">
        <div>
          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#5b4a3b]">From account</label>
          <select
            value={fromAccount}
        ...
        <div>
          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-[#5b4a3b]">To account</label>
          <select
            value={toAccount}
