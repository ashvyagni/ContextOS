...
export default function Landing({ onEnter }: { onEnter: (username: string) => void }) {
  const [username, setUsername] = useState('');

  return (
    <div className="bg-[#050505] text-white font-sans selection:bg-[#a5f3fc] selection:text-black flex flex-col">
      
      <div className="min-h-screen flex items-center justify-center bg-[#a5f3fc] text-black border-b-8 border-black">
        <div className="max-w-4xl w-full px-8 flex flex-col gap-8 relative z-10">
          ...
          >
            <input
              ...
              className="flex-1 bg-white border-4 border-black px-6 py-4 text-xl font-bold text-black placeholder-black/30 outline-none focus:ring-4 focus:ring-black/10 rounded-none transition-all"
            />
            <button 
              type="submit"
        ...
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#00000015_1px,transparent_1px),linear-gradient(to_bottom,#00000015_1px,transparent_1px)] bg-[size:32px_32px] pointer-events-none"></div>
      </div>
